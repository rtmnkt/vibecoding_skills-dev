...
要件ファイル (.local/requirements/) を起点に、条件確定→検証→実装→記録を行うワークフロー。

状態管理は CLI ツール (`uv run .github/skills/req-driven-dev/req_tool.py`) 経由で行う。
要件ファイルの内容を直接編集して状態を管理してはならない。
...
4. `uv run .github/skills/req-driven-dev/req_tool.py status XXXX` で最終状態を表示
